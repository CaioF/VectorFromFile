...
class Analyzer:
    ...
    def printResult(self, model, sample):
        ...
        for i in c.most_common(20):
            ...
            print('\n')
        print('xxx')
        print((model.wv['сука'].tolist())[:3])
        print(model.wv.similar_by_word("сука", topn=10, restrict_vocab=None))
        print('\nxxx')
        print((model.wv['бля'].tolist())[:3])
        print(model.wv.similar_by_word("бля", topn=10, restrict_vocab=None))
        print('\nзанятие')
        print((model.wv['занятие'].tolist())[:3])
        print(model.wv.similar_by_word("занятие", topn=10, restrict_vocab=None))
        print('\nматематика')
        print((model.wv['математика'].tolist())[:3])
        print(model.wv.similar_by_word("математика", topn=10, restrict_vocab=None))
        print('\nсоциология')
        print((model.wv['социология'].tolist())[:3])
        print(model.wv.similar_by_word("социология", topn=10, restrict_vocab=None))
